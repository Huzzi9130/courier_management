...
def makepayment(request):
    if request.method == 'GET':
        # Retrieve the value of buttonText from the GET request
        buttonText = request.GET.get('buttonText', '')  # This fetches the value from the GET request
        try:
            amount = int(buttonText) * 100  # Convert the buttonText to an integer and multiply by 100
        except ValueError:
            amount = 0  # Set a default value if the conversion fails
        data = {"amount": amount, "currency": "INR", "receipt": "order_rcptid_11"}

        # Perform further actions or render a response
        return render(request, 'pay.html', {'data': data})  # Replace 'payment.html' with your actual template name
    else:
        return HttpResponse("Invalid request method")
# def makepayment(request):
#     client = razorpay.Client(auth=("rzp_test_o64OGaJp8RHjib", "nnq7HUpWKup11p5hkxZycrKf"))
#     data = { "amount": (buttonText)*100, "currency": "INR", "receipt": "order_rcptid_11" }
